...
// In Vite, environment variables are accessed via import.meta.env.VITE_*
...
export const getSnakeCareAdvice = async (userQuery: string): Promise<string> => {
  try {
    ...
    if (!apiKey) {
      console.warn("Gemini API Key is missing. Please check your .env file or Vercel settings.");
      return "系統設定中：請確認 API 金鑰已正確設定。";
    }
