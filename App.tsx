...
const App: React.FC = () => {
  ...
  const handleAddSnake = (newSnake: Snake) => {
    // Note: This only updates local state, it won't push to Contentful in this version
  ...
  const handleDeleteSnake = (id: string) => {
    ...
    if (confirm('確定要下架此商品嗎？(僅本地暫時移除，不會刪除 Contentful 資料)')) {
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-concrete-500 text-sm mt-1">您目前使用 Contentful 管理資料。此處僅供預覽。</p>
            </div>
            {!isAdding && (
              <button 
              ...
              >
                <Upload size={18} /> 前往 Contentful 上架
              </button>
            )}
          </div>
          ...
          {isAdding ? (
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-xl border border-concrete-200 animate-slide-up">
              {/* Local Add Form - kept for demo purposes */}
              <div className="flex justify-between items-center mb-6 border-b border-concrete-100 pb-4">
                <h2 className="text-xl font-bold text-concrete-900 flex items-center gap-2">
                  <Upload size={20} className="text-urban-green" /> 本地預覽模式
                </h2>
